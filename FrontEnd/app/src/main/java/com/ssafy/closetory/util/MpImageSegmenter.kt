...
class MpInteractiveSegmenter(context: Context) {
    ...
    fun segment(bitmap: Bitmap, xNorm: Float, yNorm: Float): ImageSegmenterResult {
        val mpImage: MPImage = BitmapImageBuilder(bitmap).build()

        val roi = InteractiveSegmenter.RegionOfInterest.create(
            NormalizedKeypoint.create(xNorm, yNorm) // 0~1 그대로
        )
