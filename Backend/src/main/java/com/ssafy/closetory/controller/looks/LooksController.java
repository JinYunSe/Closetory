...
public class LooksController {
  ...
  public ResponseEntity<ApiResponse<VirtualFittingResponse>> requestFitting(
      @RequestBody VirtualFittingRequest request, @AuthenticationPrincipal Integer userId) {
    VirtualFittingResponse response = lookService.requestFitting(userId, request);
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(ApiResponse.ok(201, "가상 피팅 성공", response));
  }
}
